...
| **事後条件** | ログイン完了後、直前のアクションが自動的に継続・実行される |
...
6. ログイン完了後、システムがsessionStorageから保留中のアクションを取得
7. アクションに応じて以下を実行:
...
|------|------|
| **アクター** | 認証済みユーザー |
| **事前条件** | シミュレーション結果が生成済み（全体またはパーツ別） |
| **事後条件** | シェアする画像が選択された状態でカスタマイズ画面に遷移 |

**基本フロー**:
1. ユーザーがシミュレーション結果画面で「シェア」セクションを確認
2. システムが生成済みのバリエーション画像をサムネイル一覧で表示:
   - 全体シミュレーション結果
   - パーツ別適用結果（生成済みの場合）
3. ユーザーがシェアしたい画像をタップして選択
4. 選択した画像が枠線でハイライト表示される
5. ユーザーが「この画像をシェア」ボタンをタップ
6. システムがシェア画像カスタマイズ画面（SCR-010）に遷移
...
|------|------|
| **アクター** | 認証済みユーザー |
| **事前条件** | シェアする画像が選択されている |
| **事後条件** | カスタマイズされたシェア画像が生成される |

**基本フロー**:
1. システムがシェア画像カスタマイズ画面を表示
2. デフォルトでTemplate A（Before/After横並び）を選択し、プレビューを表示
3. ユーザーがテンプレートを選択:
   - **Template A**: Before/After横並び（1200x630px、OGP最適）
   - **Template B**: 単体（結果のみ、1080x1080px、Instagram最適）
   - **Template C**: パーツハイライト（1080x1350px、Instagram縦長）
4. システムが選択したテンプレートでプレビューを更新
5. ユーザーがキャプション（任意、最大140文字）を入力
6. システムがリアルタイムでプレビューを更新
7. ユーザーが「シェア画像を作成」ボタンをタップ
8. システムがシェア画像を生成し、シェア先選択画面を表示

**代替フロー**:
- 3a. Template Cを選択したがパーツ別結果がない場合
  - Template Aにフォールバックし、メッセージを表示

**テンプレート詳細**:
| テンプレート | サイズ | レイアウト | 最適SNS |
|-------------|--------|-----------|---------|
| A: Before/After | 1200x630px | 左:現在、右:理想、下部:キャプション+ロゴ | X/Twitter、OGP |
| B: 単体 | 1080x1080px | 中央:結果画像、下部:キャプション+ロゴ | Instagram投稿 |
| C: パーツハイライト | 1080x1350px | 上部:結果、右側:適用パーツリスト | Instagramストーリーズ |
