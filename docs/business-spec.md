...
**基本フロー**:
1. ユーザーが「保存」ボタンをクリック
2. システムが以下のデータをSupabase Storageに保存:
   - 現在の顔画像（current_image）
   - 理想の顔画像（ideal_image）
   - 顔全体のモーフィング結果画像（result_images）
   - パーツ別適用のベース画像（swapped_image）
   - パーツ別適用の結果画像（parts_result_image）※生成済みの場合
...
- 1a. 未認証ユーザーの場合
  ...
  - ログイン成功後、保存処理を自動継続
- 5a. 既に保存済みの場合
  - ボタンは「保存済み」状態のまま、再保存はしない

**保存データの詳細**:
| データ | 説明 |
|--------|------|
| current_image | ユーザーがアップロードした現在の顔画像 |
| ideal_image | ユーザーがアップロードした理想の顔画像 |
| result_images | 各段階（0%, 25%, 50%, 75%, 100%）のモーフィング結果 |
| swapped_image | Face Swap APIで生成したパーツ適用のベース画像 |
| parts_result_image | 選択したパーツで合成した結果画像 |
...
5. 詳細画面でシミュレーション結果を表示
6. 保存されたデータから以下を復元:
   - 顔全体のモーフィング結果（スライダーで確認可能）
   - パーツ別適用のベース画像（swapped_image）
   - パーツ別適用の結果画像（保存時に生成済みの場合）
7. ユーザーが「顔全体」「パーツ別」タブを切り替えて、保存済みの結果を即座に確認可能
8. パーツ別モードでパーツ選択を変更して再生成も可能
...
| SCR-003 | シミュレーション結果画面 | 生成結果の確認、保存、共有 | 結果画像、スライダー、顔全体/パーツ別タブ、保存ボタン、シェアボタン、ダウンロードボタン |
